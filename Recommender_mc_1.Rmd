...
set.seed(3)
split3<- sample(c(rep(0, 0.42 * nrow(MovieLense_norm)), rep(1, 0.35 * nrow(MovieLense_norm))))
...
train_3_before <- as(train_3, "matrix")

dim(train_3_before)
```
```{r}
train_3_m <- train_3_before[, col_drop]
```
...
```
```{r}
train_3_before <- train_3
train_3_m <- as(train_3_before, "matrix")
```
